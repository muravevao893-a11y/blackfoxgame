...
async function clanBonus(userId){
  try {
    const r=await pool.query('SELECT COALESCE(c.level,1) AS level FROM clans c JOIN clan_members m ON m.clan_id=c.id WHERE m.user_id=$1',[userId]);
    return r.rows[0] ? 1 + Number(r.rows[0].level || 1)*0.02 : 1;
  } catch (err) {
    if (err?.code === '42703' || err?.code === '42P01') {
      console.warn('[DB] Clan schema was old, running initDb and continuing without clan bonus once...');
      await initDb();
      return 1;
    }
    throw err;
